        ...
        <div class="px-4 py-5 sm:p-6">
          ...
          <dl class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <div class="px-4 py-5 bg-gray-50 shadow rounded-lg overflow-hidden sm:p-6">
              ...
              <dd class="mt-1 text-3xl font-semibold text-gray-900">
                {{ formatAmount(averageActiveUsers) }}
              </dd>
            </div>
          </dl>
        </div>
...
const { dailyMetrics, hourlyMetrics, loading, fetchDailyMetrics, fetchHourlyMetrics } = useTokenData()
onMounted(() => {
...
const averageDailyVolume = computed(() => {
  ...
  const sum = dailyMetrics.value.reduce((acc, curr) => acc + curr.volume, 0)
  return sum / dailyMetrics.value.length
})

const averageTransferSize = computed(() => {
  if (!dailyMetrics.value.length) return 0
  const totalVolume = dailyMetrics.value.reduce((acc, curr) => acc + curr.volume, 0)
  const totalTransfers = dailyMetrics.value.reduce((acc, curr) => acc + Number(curr.transfer_count), 0)
  return totalTransfers ? totalVolume / totalTransfers : 0
})

const averageActiveUsers = computed(() => {
  if (!dailyMetrics.value.length) return 0
  const sum = dailyMetrics.value.reduce((acc, curr) => acc + Number(curr.transfer_count), 0)
